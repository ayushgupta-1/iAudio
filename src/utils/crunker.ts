...
export default class Crunker {
  ...
  constructor({
    sampleRate,
    concurrentNetworkRequests = 200,
  }: Partial<CrunkerConstructorOptions> = {}) {
  ...
  async fetchAudio(...filepaths: CrunkerInputTypes[]): Promise<AudioBuffer[]> {
    ...
    const groups = Math.ceil(
      filepaths.length / this._concurrentNetworkRequests
    );

    for (let i = 0; i < groups; i++) {
      const group = filepaths.slice(
        i * this._concurrentNetworkRequests,
        (i + 1) * this._concurrentNetworkRequests
      );
   ...
   * Asynchronously fetches multiple audio files and returns an array of AudioBuffers.
   */
  private async _fetchAudio(
    ...filepaths: CrunkerInputTypes[]
  ): Promise<AudioBuffer[]> {
  ...
  private _interleave(input: AudioBuffer): Float32Array {
    ...
    const length = channels.reduce(
      (prev, channelData) => prev + channelData.length,
      0
    );
